...
class Connection(
    ...
    override fun getCatalog() = throw SQLFeatureNotSupportedException()
    override fun setHoldability(holdability: Int) = throw SQLFeatureNotSupportedException()
    override fun getSchema() = throw SQLFeatureNotSupportedException()
    ...
    override fun prepareStatement(
        ...
        return PreparedStatement(this, sql, parameterMeta, resultMeta)
    }
    ...
    override fun rollback() {
        val id = transaction ?: throw SQLException("No active transaction")
        try {
    ...
    internal fun <T> withTransaction(block: (transaction: ByteString?) -> T): T {
        if (transaction == null && !autoCommit) {
            transaction = txService.begin(Empty.getDefaultInstance()).id
        }
